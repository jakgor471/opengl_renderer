...
void objgl_delete(objgl2Data* data){
...
objgl2Data objgl_loadObj(const char* obj){
#if __OBJGL_DEBUG
	_debug_operations = 0;
	_debug_insertions = 0;
	...
	objgl2Material *mats = OBJGL_ALLOC(sizeof(objgl2Material) * numMaterials);

	for(unsigned int i = 0, k = 0; i < numMaterials; ++i){
		objmaterial_internal current = materials[i];
		mats[i] = (objgl2Material){&uniqueIndices[k], current.len * 3, current.name};
